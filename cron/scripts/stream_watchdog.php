...
foreach($xml_string->stream as $stream){
    ...
    $audio_sample_rate = $stream->audio_parameters->audio_sample_rate;


    if($video_codec != "copy"){$ff_video_codec = " -c:v $video_codec";}else{$ff_video_codec = " -c:v copy";}
    ...
    if($audio_codec != "copy"){$ff_audio_codec = " -c:a $audio_codec";}else{$ff_sudio_codec = " -c:a copy";}
    if($audio_bitrate != "0"){$ff_audio_bitrate = " -b:a ".$audio_bitrate."k";}else{$ff_audio_bitrate = "";}
    if($audio_channels != "0"){$ff_audio_channels = " -b:v ".$audio_channels."k";}else{$ff_audio_channels = "";}
    if($audio_samplerate != "0"){$ff_audio_samplerate = " -b:v ".$audio_samplerate."k";}else{$ff_audio_samplerate = "";}

    $ff_audio_parameters = $ff_audio_codec.$ff_audio_bitrate;

    $ffmpeg_cmd = "ffmpeg ".$input_url.$ff_video_parameters.$ff_audio_parameters." ".$output_url;
}
